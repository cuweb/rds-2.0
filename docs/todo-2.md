# TODO 2 — Component folder convention + Button POC

**Status:** Blocked on TODO 1 (needs the base stylesheet pipeline working).
...
Requirements (from the project overview):
...
- `src/components/Button/Button.tsx`
  ...
  - No icon slot, no loading state (deferred to a later component
    iteration).
- `src/components/Button/Button.scss`
  ...
  - Uses `var(--rds--color-primary)`, `var(--rds--color-white)`,
    `var(--rds--color-error)`, `var(--rds--spacing-*)`,
    `var(--rds--font-size-*)`, `var(--rds--font-weight-*)`,
    `var(--rds--radius-md)`.
...
- [ ] `Default` story renders an `<button>` with primary background and
      white text.
...
- [ ] Hover state changes color using `--rds--color-error` (matches the
      link hover rule in globals).
...
- Secondary variant colors: the project overview says there's no
  separate secondary color system. `c2b.config.json` does define
  `secondary` and `secondary-dark` colors — should the Button's
