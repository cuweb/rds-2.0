...
`src/styles/main.scss` is the single SCSS entry point that aggregates all base styles for consumers of the library. It is compiled into `dist/style.css` during the Vite build and exported via `@carletonuniversity/rds-2.0/styles`.
...
<link
  rel="stylesheet"
  href="/path/to/node_modules/@carletonuniversity/rds-2.0/dist/style.css"
/>
```
